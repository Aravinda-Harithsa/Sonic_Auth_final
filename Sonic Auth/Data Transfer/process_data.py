####################################################################
'''Sonic auth code for decoding the input information'''
#####################################################################
# Include the below files in main function of yours. 
...
def process_data_final(name):

    data, samplerate = soundfile.read(name)
    soundfile.write(name, data, samplerate)
    output_data=[]
    ...
    def process_bits():
        while True:
            ...
            output_data.append(psk.decode(cur_bits[:-len(sigil)]))
...
##############################
    '''Call this function by passing name of the wav file / with location  like shown below '''


data1=process_data_final("Rec1.wav")
print(data1)
